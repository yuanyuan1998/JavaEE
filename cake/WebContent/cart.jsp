...
<div class="wd" id="buy1">
        ...
        <tbody><tr>
        	...
        	<c:forEach items="${cartlist }" var="c" >
        		<tr indexpid="11" id="li1-11">
        			<td><input name="checkbox" type="checkbox" value="${c.product.id },${c.size },${c.count }" /></td>
                    ...
                    <td valign="middle"><span class="dd_num2" style="display:none;">${c.count }</span><span class="dd_num"><a onclick="jian('${c.product.id}','${c.size }','${c.count }')" href="javascript:">-</a><input name="" type="text" value="${c.count }"><a onclick="add('${c.product.id}','${c.size }','${c.count }')" href="javascript:">+</a></span></td>
                    <td class="xj" valign="middle">${c.product.price*c.count}</td>
                    <td mid="11"><a href="javascript:" onclick="delCar('${c.product.id}','${c.size }','${c.count }')" class="pro_del">删除</a></td>
                </tr>
        	</c:forEach>
    </tbody></table>
    <div class="pj_box">
    ...
    <script type="text/javascript">
	    ...
	    function jian(id,size,count){
	    	$.ajax({
                type : "post",
                url : "<%=path %>/cart/jianCount?id="+id,
                data:{size:size,count:count}
    		});
    		location.reload();
	    };
	    function add(id,size,count){
	    	$.ajax({
                type : "post",
                url : "<%=path %>/cart/addCount?id="+id,
                data:{size:size,count:count}
    		});
    		location.reload();
	    };
    	function delCar(id,size,count){//删除商品
    		$.ajax({
                type : "post",
    ...
    <div class="d_dan_result" >
        <h2>您需为订单支付：￥<span id="total"></span></h2>
        <div class="d_dan_order_btn" id="confirm">
        	...
        	<a class="submit_order" href="javascript:void(0);" onclick="buy1()">确认</a></div>
    </div>
</div>
<script type="text/javascript">
	function buy1(){
