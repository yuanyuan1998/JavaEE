...
<form action="http://www.dingdangao.net.cn/Product/List1" id="list_form_type" method="post">                <input type="hidden" name="typeid" id="typeid">
    ...
    <div class="container pro_a">
        <div class="row">
        	<c:forEach items="${flp }" var="flp">
        		<div class="lista col-lg-3">
                        <a class="list_pica" href="<%=path %>/product/Product?id=${flp.id }">
                            <img src="${flp.img1 }">
                        </a>
                        <div class="lista_text">
                            <div class="list_bt0">
                                <i class="icon_cu"><img src="<%=path %>/images/tag_new_38.png" width="39" height="16"></i>
                                <a class="bta" href="<%=path %>/product/Product?id=${flp.id }"><h2>${flp.name }</h2></a>
                                <span>￥ ${flp.price }</span>
                            </div>
                            <div class="list_bt1">
                                <span>￥ ${flp.price }</span>
                                <small class="">${flp.description }</small>
                                <ins><a class="btn_buya" href="<%=path %>/product/Product?id=${flp.id }">立即购买</a></ins>                         
                            </div>
                        </div>
                    </div>
        	</c:forEach>
